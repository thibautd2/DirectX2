...
/*	if(error != 0)
	...
	*/
	...
	fieldOfView = (float)3.14159 / 4.0f;
	screenAspect = (float)screenWidth / (float)screenHeight;

	// Create the projection matrix for 3D rendering.
	m_projectionMatrix.CreatePerspectiveFieldOfView(fieldOfView, screenAspect, screenNear, screenDepth);
    // Initialize the world matrix to the identity matrix.
	m_worldMatrix = m_worldMatrix.Identity;

	// Create an orthographic projection matrix for 2D rendering.
	m_orthoMatrix.CreateOrthographic((float)screenWidth, (float)screenHeight, screenNear, screenDepth);
